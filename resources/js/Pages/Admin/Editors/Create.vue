...
const submit = () => {
    editor.post(route('admin.editors.invite'));
}
</script>
...
<template>
    ...
    <AdminLayout type="admin">
        <div class="w-full py-8 px-4 sm:px-6 lg:px-8">
            <h3 class="text-lg leading-6 font-medium text-gray-900"> Invite Editor </h3>
                        ...
                        <div>
                            <div class="space-y-6 sm:space-y-5">
                                <div class="px-6 sm:grid sm:grid-cols-5 sm:gap-4 sm:items-start sm:pt-5">
                                    <BreezeLabel for="email" value="Editor email" class="sm:mt-px sm:pt-2" />
                                    <div class="mt-1 sm:mt-0 sm:col-span-4">
                                        <BreezeInput type="text" v-model="editor.email" id="email" class="block w-full" />
                                        <BreezeInputError class="mt-1" :message="editor.errors.email" />
                                    </div>
                                </div>
                                <div class="px-6 sm:grid sm:grid-cols-5 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-5">
                                    <BreezeLabel for="name" value="Editor name" class="sm:mt-px sm:pt-2" />
                                    <div class="mt-1 sm:mt-0 sm:col-span-4">
                                        <BreezeInput type="text" v-model="editor.name" id="name" class="block w-full" />
                                        <BreezeInputError class="mt-1" :message="editor.errors.name" />
                                    </div>
                                </div>
                            </div>
                        </div>
