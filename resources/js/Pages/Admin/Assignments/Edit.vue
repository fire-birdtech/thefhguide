...
const findEditor = () => {
    return props.editors.find(editor => editor.id === props.currentAssignment.user.id)
}

const selected = ref(findEditor());
...
const submit = () => {
    assignment.user_id = selected.value.id;
    assignment.post(route('admin.assignments.update', [assignment.id]));
}
</script>
                ...
                <form @submit.prevent="submit" class="space-y-4 divide-y divide-gray-200">
                    ...
                    <div class="px-6 sm:grid sm:grid-cols-5 sm:gap-4 sm:items-start sm:border-t sm:border-gray-200 sm:pt-4 sm:pb-2">
                        ...
                        <div class="mt-1 sm:mt-0 sm:col-span-4">
                            <FormText v-model="assignment.assignable.name" id="assignable" class="block w-full" />
                        </div>
                    </div>
                    <div class="px-6 py-4">
                        <div class="flex justify-end">
                            <SecondaryButton :href="route('admin.assignments.index')" as="link">Cancel</SecondaryButton>
