    ...
    <tbody class="bg-white">
        <tr v-for="(item, rowIdx) in rows" :key="rowIdx" :class="rowIdx % 2 === 0 ? undefined : 'bg-gray-50'">
            <td v-for="(cell, cellKey, cellIdx) in cells" :key="cellKey" :class="[cellIdx === 0 ? 'pl-4 pr-3 sm:pl-6' : 'px-3', 'whitespace-nowrap py-2.5 text-sm text-gray-500']">
                <Link v-if="cellIdx === 0" :href="route(`editor.${routeType}.show`, [item.slug ? item.slug : item.id])" class="text-gray-900 hover:text-gray-700 font-semibold">{{ item[cellKey] }}</Link>
                <Badge v-else-if="cellKey === 'status'" :status="item.status" />
            ...
            <td v-if="actions" class="flex justify-end whitespace-nowrap py-3 pl-3 pr-4 text-right text-sm font-medium space-x-2 sm:pr-6">
                <Link :href="route(`editor.${routeType}.show`, [item.slug ? item.slug : item.id])" class="text-indigo-600 hover:text-indigo-900">
                    <EyeIcon class="h-6 w-6" />
                    <span class="sr-only">View {{ item.name }}</span>
                </Link>
                <Link v-if="! item.locked" :href="route(`editor.${routeType}.edit`, [item.slug ? item.slug : item.id])" class="text-indigo-600 hover:text-indigo-900">
